...
<div class="flex justify-center items-center min-h-screen">
    <div 
       x-data="{
        feedbackquestion: {{ $feedbackcat->feedbackquestion->count() ? $feedbackcat->feedbackquestion->map(fn($q) => [
            'id' => $q->id,
        ...
        class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-2xl">
        ...
        <form action="{{ route('feedback-category.update', $feedbackcat->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div>
                ...
                <input type="text" id="category_name" name="category_name"
                       value="{{ old('category_name', $feedbackcat->category_name) }}"
            ...
            <template x-for="(question, index) in feedbackquestion" :key="index">
                <div class="p-4 border border-gray-200 rounded-lg shadow-sm bg-gray-50 relative space-y-3 mt-3">
                    <input type="hidden" :name="'feedbackquestion[' + index + '][id]'"  :value="question.id">
                    <div>
                    ...
                    <!-- Remove Button -->
                    <button type="button" 
                            x-on:click="if(question.id) removeIds.push(question.id);
                            feedbackquestion.splice(index, 1)"
                            class="absolute top-2 right-2 text-red-500 hover:text-red-700">
                        <i class="bi bi-x-circle text-xl"></i>
                    </button>
                    <input type="hidden" name="removeIds" :value="removeIds.join(',')">
                </div>
            </template>
            ...
            <div class="text-right">
                <button type="button"
                        x-on:click="feedbackquestion.push({ id: null, question_text: '', input_type: '' })"
